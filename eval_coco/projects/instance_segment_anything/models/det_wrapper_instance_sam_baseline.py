...
from projects.instance_segment_anything.models.segment_anything import sam_model_registry_baseline, SamPredictor
from .focalnet_dino.focalnet_dino_wrapper import FocalNetDINOWrapper
from .hdetr.hdetr_wrapper import HDetrWrapper


@DETECTORS.register_module()
class DetWrapperInstanceSAM_baseline(BaseDetector):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(DetWrapperInstanceSAM_baseline, self).__init__(init_cfg)
    ...
    def simple_test(self, img, img_metas, rescale=True, ori_img=None):
        ...
        results = self.det_model.simple_test(img,
                                             img_metas,
                                             rescale)
        results[0]['boxes'] = results[0]['boxes'][:100]
        results[0]['scores'] = results[0]['scores'][:100]
        results[0]['labels'] = results[0]['labels'][:100]
        ...
        # mask_pred_binary = (mask_pred > self.predictor.model.sam.mask_threshold).float()
        if self.use_sam_iou:
